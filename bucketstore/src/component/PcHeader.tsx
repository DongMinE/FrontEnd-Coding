...
export default function PcHeader() {
  const linksSection1 = ["클리어런스", "아울렛", "브랜드", "여성", "남성", "골프", "아웃도어&스포츠"];

  const linksSection2 = ["런칭", "베스트", "스타일", "기획전", "이벤트"];
    ...
    <div className={classes.container}>
      <div className={classes.headerDiv}>
        ...
        <section className={classes.navMenuDiv}>
          ...
          <section className={classes.detailDiv2}>
            {linksSection2.map((text, index) => (
              <div key={index}>
                <Link to="/">{text}</Link>
              </div>
            ))}
